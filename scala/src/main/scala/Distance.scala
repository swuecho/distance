object Hm {
  ...
  def csf_cdr3_vs_pb_cdr3(csf_cdr3: String, pb_cdr3s: List[String]): Unit = {
    val cdr3 =
      pb_cdr3s.filter(x => hamming_distance(cdr3_from_line(x), csf_cdr3, 2))
    Files.write(
        Paths.get("/home/hwu/dev/distance/result_scala/" + csf_cdr3 + ".csv"),
        cdr3.mkString("\n").getBytes())
...
object Distance extends App {
  ...
  csf_lines.toList.par.foreach(x =>
        Hm.csf_cdr3_vs_pb_cdr3(Hm.cdr3_from_line(x), pb_lines))
}
